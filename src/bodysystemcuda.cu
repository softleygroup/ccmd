                    ...
                    typename vec4<T>::Type bj) 
{
    ...
    // s = q_i * q_j * invDistCube [1 FLOP]
    T s = bi.w * bj.w * invDistCube;
...
integrateBodies(typename vec4<T>::Type* newPos, 
                ...
                float deltaTime, int totalNumBodies)
{
                          ...
                          int q)
{
    ...
    for (unsigned int dev = 0; dev != numDevices; dev++)
    {
        ...
        // Determine optimum number of thread blocks
