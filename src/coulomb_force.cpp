...
Coulomb_force::Coulomb_force(const IonCloud_ptr ic, const SimParams& sp)
...
void Coulomb_force::update() {
    // direct_force();
    // Start calculating force in another thread so the integrator can get on
    // with the fast force calculation. Create a local copy of the ion
    // positions so we can work on them without them changing
    ...
    switch (sim_params.coulomb_threads) {
        ...
        case 1:
            ...
            break;
        default:
            for (int i_thread = 0; i_thread < sim_params.coulomb_threads;
                    i_thread++) {
                threads.push_back(std::thread(
                            &Coulomb_force::split_force, this, i_thread));
            }
            break;
    }
}
...
void Coulomb_force::split_force(int n) {
    ...
    int imin = std::floor(static_cast<double>(n)/static_cast<double>(max_thread)
            * ionsCopy.size());
    int imax = std::floor(
            static_cast<double>(n+1)/static_cast<double>(max_thread)
            * ionsCopy.size());
    assert(imin >= 0);
